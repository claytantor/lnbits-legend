...
@check_user_exists()
async def index():
    return await render_template("rapaygo/index.html", user=g.user)


@rapaygo_ext.route("/<paymentid>")
async def displaypin(paymentid):
    ...
    if not is_paid:
        return await render_template("rapaygo/error.html", pin="filler", not_paid=True)
    await update_payment_status(checking_id=lnurlpospayment.payhash, pending=True)
    return await render_template(
        "rapaygo/paid.html", pin=lnurlpospayment.pin, not_paid=True
    )
